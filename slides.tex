...
\begin{itemize}
	...
	\item gambling ECs: $\mathbb{E}(\mathrm{MP}) = 0$ and it is positively and negatively weight divergent \pause
...
\begin{frame}
	...
	\begin{center}%#2
		...
		% First we draw the action dots
		\foreach \pos/\name in { {(0.3,2)/{a0}}, {(1.3,0.3)/{a1}}, {(1.3,-0.3)/{b1}} }
		\node[acdot] (\name) at \pos {};
		
		
		% First we draw the vertices
		\foreach \pos/\name in {{(0,2)/0}, {(1,0)/1}, {(3,0)/3}}
		\node[vertex] (\name) at \pos {$s_{\name}$};
